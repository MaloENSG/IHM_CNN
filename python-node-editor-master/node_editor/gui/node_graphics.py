...
class Node_Graphics(QtWidgets.QGraphicsItem):
    ...
    def build(self):
        ...
        for pin in self._pins:
            ...
            if pin.is_output and pin.execution:
                height_input += pin_dim["h"]
            if not pin.is_output and pin.execution:
                height_output += pin_dim["h"]
            if not pin.execution:
                height_param += pin_dim["h"]
        total_height += height_param + max(height_input, height_output)
        ...
        if pin_dim:
            ...
            ybis = bg_height - total_height / 2 - 10
            ...
            for pin in self._pins:
                ...
                if pin.is_output:
                    ybis += pin_dim["h"]
                    pin.setPos(total_width / 2 - 10, ybis)
