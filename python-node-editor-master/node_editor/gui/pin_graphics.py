...
class Pin_Graphics(QtWidgets.QGraphicsPathItem):
    ...
    def paint(self, painter, option=None, widget=None):
        if self.execution:
            ...
            if self.pintype == "int":
                painter.setPen(QtGui.QColor(66,215, 90))
            elif self.pintype == "rate":
                painter.setPen(QtGui.QColor(220,20, 240))
            elif self.pintype == "list2":
                painter.setPen(QtGui.QColor(66, 90, 215))
            elif self.pintype == "activation":
        ...
        if self.is_connected():
            if self.execution:
                ...
                if self.pintype == "int":
                    painter.setBrush(QtGui.QColor(66,215, 90))
                elif self.pintype == "rate":
                    painter.setBrush(QtGui.QColor(220,20, 240))
                elif self.pintype == "list2":
                    painter.setBrush(QtGui.QColor(66, 90, 215))
                elif self.pintype == "activation":
                    painter.setBrush(QtGui.QColor(240, 160, 20))
                else :
                    painter.setPen(QtCore.Qt.white)
                #####
